...
api_v1_urlpatterns = [
    path(
        ...
        include(("repks.apps.accounts.api.v1.urls", "accounts"), namespace="api-v1-accounts"),
    ),
    path(
        f"{API_PREFIX}/v1/hello_repka/",
        include(("repks.apps.hello_repka.api.v1.urls")),
    ),
]
...
# enable Swagger
if "SWAGGER" in settings.REPKS_FEATURES:
    ...
    api_v1_schema_view = get_schema_view(
        openapi.Info(
            title="repks",
            default_version="v1",
            description="repks API v1 description",
        ),
        public=True,
